...
const Register = () => {
    ...
    const handleRegister = (e) =>{
		...
		if(!/^(?=.*[A-Z])(?=.*[a-z]).{6,}$/.test(password)){
			setError("password length should be more than 6 characters and have atleast one uppercase and lowercase letter")
		}
	...
	<form onSubmit={handleRegister} noValidate="" action="" className="space-y-3">
    <div className="space-y-1 text-sm">
			<label htmlFor="username" className="block ">Name</label>
			<input type="text" name="name" id="name" required placeholder="Your Name" className="w-full px-4 py-3 rounded-md border-2 border-black text-black" />
		</div>
		<div className="space-y-1 text-sm">
			<label htmlFor="username" className="block ">Email</label>
			<input type="email" name="email" id="email" required placeholder="Your Email" className="w-full px-4 py-3 rounded-md border-2 border-black text-black" />
		</div>
        <div className="space-y-1 text-sm">
			<label htmlFor="username" className="block ">Photo URL</label>
			<input type="text"  name="photoUrl" id="photoUrl" placeholder="Your Photo URL" className="w-full px-4 py-3  border-2 rounded-md border-black text-black" />
		</div>
		<div className="space-y-1 text-sm  ">
...
<div>
<button className="block w-full p-3 text-center rounded-md text-white bg-[#003C43] hover:bg-white hover:border hover:text-black hover:border-black">Register</button>
	...
	<p className="text-md text-center sm:px-6 ">Already have an account?
		<Link to='/login' rel="noopener noreferrer" href="#" className="underline font-bold"> Login</Link>
	</p>
</div>
        </div>
    );
